...
func (s *upgradedReadSource) ReportCatalog(ctx context.Context, in model.ReportCatalogRequest, a sdk.Authority) (model.ReportCatalog, error) {
	out, err := s.sourceFixture.ReportCatalog(ctx, in, a)
	out.ReadProof = "new-owner-read-attestation"
	return out, err
}
func (s *upgradedReadSource) AuthorizeReportResultRead(context.Context, model.ReportQueryResultAuthorization, sdk.Authority) error {
	s.readChecks++
	return s.denial
}
func (s *upgradedReadSource) AuthorizeReportCatalogRead(context.Context, model.ReportCatalogReadAuthorization, sdk.Authority) error {
	s.readChecks++
	return s.denial
}

func TestReportLegacyResultsRetainExecutionChecksAfterOwnerUpgrade(t *testing.T) {
	for _, arguments := range []string{`{"operation":"query","report_key":"sales"}`, `{"operation":"catalog"}`} {
		t.Run(arguments, func(t *testing.T) {
			adapter, selected, original, request, grant := toolFixture(t)
